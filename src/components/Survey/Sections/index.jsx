...
const Sections = () => {
  ...
  const handleDelete = (id) => {
    axios
      .delete(`/api/survey/deleteSection/${id}`, {
        headers: { authorization: `Bearer ${appState?.accessToken}` },
      })
      .then((res) => {
  ...
  const handleEdit = (id) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        const data = {
          ...res?.data?.data,
          questions: JSON.parse(res?.data?.data?.questions),
          conditions: JSON.parse(res?.data?.data?.conditions),
        };
